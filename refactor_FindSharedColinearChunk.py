...
def MakeGenomeDiagramDirecotry():
    """We need a place to store all genome diagrams. To do this we must create
    a directory. This will check for an existing directory. If it exists it
    will ask if you want to delete everthing in it and overwrite
    :returns: 
...
class MCScanAlign(object):
    ...
    def ExtractGenomicRegions(self, InputGeneDictionary):
        ...
        for genelist in CreateDiffList:
            ...
            elif genelist[0] != '||' and genelist[1] != '||':
                BaseStorage = genelist[1][0:3]
                if BaseStorage in InputGeneDictionary.keys():
                    pass
    ...
    def ExtractNucloTides(self, LoadedFiles):
        """TODO: Docstring for ExtractNucloTides.
        :returns: TODO
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "i:c:b:p:o:h", ["input",
            "basenames", "collineage", "prefix", "output", "help"])
